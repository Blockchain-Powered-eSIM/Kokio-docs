...
/**
 * Where to read the docs from.
 *
 * A preview deployment has to read its own copy, or it answers from whatever
 * is on the live site and a change cannot be reviewed before it ships.
 */
function fullTextUrl() {
  const origin = process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : SITE_URL;
  return `${origin}/llms-full.txt`;
}
...
/** Fetches the published docs, reusing the last copy for an hour. */
export async function loadSections({ fetchImpl = fetch, now = Date.now() } = {}) {
  ...
  const sections = parseSections(await response.text());
...
async function askModel(prompt, apiKey, fetchImpl) {
  ...
  if (response.status === 429) return { degraded: "quota" };
  if (!response.ok) return { degraded: "unavailable" };
  ...
  return text ? { answer: text } : { degraded: "unavailable" };
}
 ...
 */
export async function answerQuestion(question, { apiKey, fetchImpl = fetch } = {}) {
  const asked = String(question ?? "").trim().slice(0, MAX_QUESTION_LENGTH);
  if (!asked) throw new Error("empty question");

  const sections = await loadSections({ fetchImpl });
  ...
  if (!apiKey) return { answer: null, sources, degraded: "unavailable" };

  const result = await askModel(buildPrompt(asked, hits), apiKey, fetchImpl);
  return { answer: result.answer ?? null, sources, degraded: result.degraded };
}
