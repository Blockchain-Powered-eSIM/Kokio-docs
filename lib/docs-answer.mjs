...
async function askModel({ prompt, apiKey, fetchImpl, now = () => Date.now() }) {
  ...
  for (const model of MODELS) {
    ...
    if (last.answer) return last;
    // Only a model-side failure is worth carrying to the other model. A
    // rejected key, an exhausted quota or a refused request follows the
    // account, not the model, so trying again just doubles the damage.
    const modelSide = last.status === 0 || last.status === 408 || last.status >= 500;
    if (!modelSide) break;
  }
