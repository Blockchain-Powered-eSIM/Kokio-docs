...
// A used-up quota is tried once on each model, since the limits are counted
// per model, then given up on with the pages still returned.
resetCache();
...
assert.equal(quota.sent.length, 2);
assert.notEqual(quota.sent[0].url, quota.sent[1].url, "the second try uses the other model");
assert.ok(outOfQuota.sources.length > 0);

// Quota on the first model, room on the second.
resetCache();
const spilled = fakeFetch(failure(429, "RESOURCE_EXHAUSTED"), ok("A passkey is a key."));
const answeredByFallback = await ask("how do passkeys work", {
  apiKey: "test-key",
  fetchImpl: spilled,
});
assert.equal(answeredByFallback.answer, "A passkey is a key.");
...
console.log("docs-answer: passed. Request shape pinned, parsing, ranking and 12 behaviours checked.");
